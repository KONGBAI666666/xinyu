...
- Ollama 等本地/自建服务: 需通过 EMBEDDING_MODEL / EMBEDDING_DIM 环境变量指定

显式配置了 EMBEDDING_MODEL 时, 覆盖自动映射 (优先级最高)。
"""
...
def resolve_embedding_profile(model_config: ModelConfig) -> EmbeddingProfile:
    ...
    if settings.embedding_model_override:
        dim = settings.embedding_dim_override or DEFAULT_FALLBACK_DIM
        return EmbeddingProfile(settings.embedding_model_override, dim)
    ...
    if provider == "ollama":
        raise AiError(
            ...
            "然后在 .env 配置 EMBEDDING_MODEL=bge-m3 与 EMBEDDING_DIM=1024 后重试。",
            code=EMBEDDING_CONFIG_ERROR,
        )
    ...
    if profile is None:
        raise AiError(
            ...
            "请在 .env 配置 EMBEDDING_MODEL 与 EMBEDDING_DIM 后重试。",
            code=EMBEDDING_CONFIG_ERROR,
        )
    return profile
